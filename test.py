...
# Cho phép override mức log bằng biến môi trường LOG_LEVEL (DEBUG/INFO/...)
...
async def get_user_jwt(user_email: str, redis):
    ...
    return None
# ==================== AGENT EXECUTION ====================
async def invoke_agent(user_id: str, user_input: str, job_id: str, redis: aioredis.Redis, jwt_override: Optional[str] = None) -> str:
    """
    ...
    try:
        ...
        # Prefer JWT passed directly with the job (avoids "stuck"/race conditions).
        if jwt_override:
            jwt = jwt_override
        else:
...
async def handle_message(
    ...
    try:
        ...
        # Gọi agent - prefer jwt included in payload to avoid stale/missing Redis entry
        jwt_from_payload = payload.get("jwt")
        if jwt_from_payload:
            logger.debug(f"🔐 Using JWT from payload for job {job_id} (user {user_id})")
        else:
            logger.debug(f"🔍 No JWT in payload for job {job_id}; falling back to Redis")

        reply = await invoke_agent(user_id, user_message, job_id, redis, jwt_override=jwt_from_payload)
...
async def send_message(request: Request): # nhận toàn bộ request từ client
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            headers = {"X-Service-Api-Key": config.SERVICE_API_KEY} if config.SERVICE_API_KEY else None
            ...
            # Extract authenticated user identifier from backend
            userName = response.text.strip()

            # JWT debug printing is gated by environment variable to avoid leaking tokens in production
            debug_print_jwt = os.getenv("DEBUG_PRINT_JWT", "false").lower() == "true"
            if debug_print_jwt:
                # explicit raw print for debugging (goes to stdout / container logs)
                print(f"JWT_RAW: {jwt_token}")
            else:
                # mask token in normal logs
                masked = jwt_token[:8] + "..." + jwt_token[-8:] if len(jwt_token) > 24 else jwt_token[:8] + "..."
                logger.info(f"🔐 /send_message authenticated: user={userName} (jwt={masked})")
            ...
            try:
                ...
                payload = {
                    ...
                    "user_id": user_id,
                    "text": text,
                    # Include the exact JWT used by this request so workers don't rely solely on Redis
                    "jwt": jwt_token,
                    "timestamp": datetime.now(timezone.utc).isoformat()
                }
                ...
                # Return user identifier; JWT was printed to stdout for realtime debugging
                return JSONResponse({
                    "status": "ok",
                    "job_id": job_id,
                    "queue": queue_name,
